class Pic < ApplicationRecord
  ...
  def self.data_report_people_division(start_date, end_date)
    query = [" SELECT d.name as division_name, c.name as pic, SUM(A.total_norm_and_qty_per_area) as eh, SUM(A.total_wh) as wh, SUM(A.total_norm_and_qty_per_area)/SUM(A.total_wh)*100 AS nilai_p FROM
    	(SELECT t_wh.area_id, total_wh, total_norm_and_qty_per_area FROM (
      		SELECT area_id, SUM(wh) as total_wh FROM twhs WHERE PIC_DATE BETWEEN CAST(? AS DATE) AND CAST(? AS DATE)
      		GROUP BY area_id
    	) AS t_wh
    ...
    GROUP BY twh_id
      ...
      GROUP BY area_id
    ) AS t_qty ON t_qty.area_id = t_wh.area_id) A INNER JOIN areas b ON A.area_id = b.id INNER JOIN  divisions d ON d.id = b.division_id INNER JOIN employees c ON d.employee_id = c.id", start_date, end_date, start_date, end_date]
    results = Pic.find_by_sql(query)
  end
