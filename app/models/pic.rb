class Pic < ApplicationRecord
  ...
  def self.data_report_people_area(start_date, end_date)
    query = ["
      SELECT b.name as area, c.name as pic, A.total_norm_and_qty_per_area as eh, A.total_wh as wh, A.total_norm_and_qty_per_area/A.total_wh*100 AS nilai_p
        FROM (
          SELECT t_wh.area_id, total_wh, total_norm_and_qty_per_area
          FROM (
            SELECT area_id, SUM(wh) as total_wh FROM twhs
      ...
      JOIN
        (SELECT area_id, SUM(total_qty) AS total_norm_and_qty_per_area
          FROM (
            ...
            FROM areas a inner join employees b ON a.employee_id = b.id INNER JOIN twhs c ON a.id = c.area_id INNER JOIN pics d ON d.twh_id = c.id
          JOIN (
            SELECT pics.id as id, norms, qty FROM parts JOIN pics ON parts.id = pics.part_id) e ON e.id = d.id
          WHERE c.PIC_DATE BETWEEN CAST(? AS DATE) AND CAST(? AS DATE)
          GROUP BY twh_id
          )  AS total_qty_table
          GROUP BY area_id
        ...
        ORDER BY b.name", start_date, end_date, start_date, end_date]
    results = Pic.find_by_sql(query)
  end

  def self.data_report_people_division(start_date, end_date)
    query = ["
      SELECT d.name as division_name, c.name as pic, SUM(A.total_norm_and_qty_per_area) as eh, SUM(A.total_wh) as wh, SUM(A.total_norm_and_qty_per_area)/SUM(A.total_wh)*100 AS nilai_p
        FROM(
          SELECT t_wh.area_id, total_wh, total_norm_and_qty_per_area
          FROM(
            SELECT area_id, SUM(wh) as total_wh FROM twhs WHERE PIC_DATE BETWEEN CAST(? AS DATE) AND CAST(? AS DATE)
        ...
        GROUP BY b.division_id
        ORDER BY d.name", start_date, end_date, start_date, end_date]
    results = Pic.find_by_sql(query)
  end

  def self.data_report_people_area_division(start_date, end_date)
    query = ["
      SELECT b.name as area, c.name as pic, A.total_norm_and_qty_per_area as eh, A.total_wh as wh, A.total_norm_and_qty_per_area/A.total_wh*100 AS nilai_p
        FROM (
          SELECT t_wh.area_id, total_wh, total_norm_and_qty_per_area
  ...
  def self.data_report_people_division_test(start_date, end_date)
    query = ["
      SELECT d.name as division_name, c.name as pic, SUM(A.total_norm_and_qty_per_area) as eh, SUM(A.total_wh) as wh, SUM(A.total_norm_and_qty_per_area)/SUM(A.total_wh)*100 AS nilai_p
        FROM(
          SELECT t_wh.area_id, total_wh, total_norm_and_qty_per_area
